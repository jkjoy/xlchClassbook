	...
	$(function() {
		...
		if (!WebUploader.Uploader.support('flash') && WebUploader.browser.ie) {
			...
			if (flashVersion) { (function(container) {
					...
					var swf = 'assets/swf/expressInstall.swf';
					// insert flash object
		...
		uploader = WebUploader.create({
			...
			chunked: false,
			formData: Upload_formData,
			server: Upload_Url,
			// runtimeOrder: 'flash',
			accept: (navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i) ? {
				//给某些垃圾蓝绿大厂的手机做兼容，tmd上传图片打开照相机？傻逼吗。
				title: 'File'
			} : {
				title: 'Images',
		...
		uploader.on('uploadFinished',Upload_finished);
		uploader.on('uploadAccept', function(file, data) {
			if(data.Code < 1){
				notify('['+file.file.name+']'+data.Message, 'danger');
			}
		});
		...
		$info.on('click', '.ignore', Upload_finished);
